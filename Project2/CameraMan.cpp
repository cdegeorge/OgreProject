...
bool CameraMan::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
	if (mStyle == CS_FREELOOK)
	{
		...
		if (mVelocity != Ogre::Vector3::ZERO)
			mCamNode->translate(mVelocity * evt.timeSinceLastFrame, Ogre::SceneNode::TS_LOCAL);
	}
...
#endif
{
	...
	{
		static Ogre::Real degree = 0;
		const Ogre::Real min = -45.0f;
		const Ogre::Real max = 75.0f;

		mCamNode->yaw(Ogre::Degree(-evt.state.X.rel * 0.15f));

		degree += evt.state.Y.rel * 0.15f;
		if (-degree < min) degree = -min;
		else if (-degree > max) degree = -max;
		else mCamera->pitch(Ogre::Degree(-evt.state.Y.rel * 0.15f));
	}
}
